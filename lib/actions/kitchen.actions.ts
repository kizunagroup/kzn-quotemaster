...
// Server action to get eligible kitchen managers (PERFORMANCE OPTIMIZED: search + limit)
export async function getKitchenManagers(searchQuery?: string): Promise<{ id: number; name: string; email: string; department: string | null; jobTitle: string | null; role: string; }[] | { error: string }> {
  try {
