...
const fetchProducts = async (url: string): Promise<ProductsResponse> => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Không thể tải danh sách hàng hóa: ${response.status} ${response.statusText}`
    );
  }
...
// Task 3A.3: Main SWR hook for product data management with URL state integration
export function useProducts() {
  ...
  // Configure SWR with optimized settings for product data
